...
class SociosConnection(models.Model, Suscriber):
	...
	def prepare_row_fields(self, row):
		...
		if row.fec_inen:
			...
			ingresos = self.env["infocoop_ingresos"].search([("medidor","=",row.medido),("orden","=",row.orden)],limit=1)
			if ingresos and ingresos.fec_ingr:
				...
				if red_usu and red_usu.fec_inst:
					...
					modi_soc = self.env["infocoop_modi_soc"].search([("medidor","=",row.medido),("orden","=",row.orden),("campo","=","ACTIVO"),("actual","=","S")],order="fecha desc, hora desc",limit=1)
					if modi_soc:
	...
	def get_or_create_service_category(self, row):
		...
		sc = self.env["electric_utility.service_category"].search([("code","=",code),],limit=1)
		if sc:
			return sc
		else:
			return self.env["electric_utility.service_category"].create({"code":code, "ersep_code": ersep_code, "name":name})
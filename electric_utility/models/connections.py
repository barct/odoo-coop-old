...
class Connection(models.Model):
	...
	sector_id = fields.Many2one("electric_utility.sector", "Sector", required=True)
	...
	meter_type = fields.Selection(string="Meter Type", selection=[('A', 'Analogic'),('D','Digital')])
...
class Contrat(models.Model):
	...
	connection_id = fields.Many2one("electric_utility.connection", string = "Connection", required=True)
	contrat_number = fields.Char("Contrat N°",length=7)
	client_id = fields.Many2one("res.partner", string="Client", required=True)


	service_category_id = fields.Many2one("electric_utility.service_category", string="Service Category", required=True)
	service_status = fields.Selection(string="Service Status", selection=[(1, 'Normal'),(2,'In Cutlist'),(3,'Suspended'),(4,'Inactive')], default=0)
	installed_potency = fields.Integer("Installed Potency", default=5)
	
	billing_sequence = fields.Float("Billing Sequence")
	billing_group_id = fields.Many2one("electric_utility.billing_group", string="Billing Group")
